...
public:
	...
	string convertDate(Date myDate)
	{
		ostringstream oss;
		oss << setfill('0') << setw(2) << myDate.date << '/'
			<< setw(2) << myDate.month << '/' << myDate.year;
	...
	string convertToString(vector<string> &input)
	{
		ostringstream oss;
		oss << setfill('0');
		for(auto &ite:input)
